...
tree = ET.parse('/home/lcastri/git/PeopleFlow/HRISim_docker/pedsim_ros/pedsim_simulator/scenarios/warehouse_complete.xml')
root = tree.getroot()
...
OUTCSV_PATH= os.path.expanduser(f'utilities_ws/src/RA-L/hrisim_postprocess/csv/HH/my{"_noised" if ADD_NOISE else "_nonoise"}/')
...
for bag in BAGNAME:
    ...
    for tod in TOD:
        ...
        for wp in wps:
            ...
            BACs[wp] = np.clip(np.round(BACs[wp], 2), 0, 100)  # Ensure BAC remains between 0 and 100 with 2 decimal precision
