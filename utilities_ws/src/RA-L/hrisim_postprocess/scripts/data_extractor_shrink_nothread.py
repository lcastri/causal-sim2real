...
class DataManager():
    ...
    def __init__(self, x = 0, y = 0, gx = NOGOAL, gy = NOGOAL):
        ...
        rospy.Subscriber("/hrisim/robot_past_bac", BatteryAtChargers, self.cb_robot_past_bac)
    ...
    def cb_robot_past_bac(self, bacs: BatteryAtChargers):
        for bac in bacs.BACs:
            self.pastBACs[bac.WP_id.data] = bac.BAC.data
...
def save_data_to_csv(data_rows, filename, csv_path, wps):   
    ...
    for wp_id in wps.keys():
        wp_df = pd.DataFrame(
            [{key: row[key] for key in row if key in (general_columns_name + [f"{wp_id}_NP", f"{wp_id}_PD", f"{wp_id}_BAC", f"{wp_id}_pBAC"])} for row in data_rows]
        )
        
        # Rename the WP-specific columns
        wp_df = wp_df.rename(columns={f"{wp_id}_NP": "NP", f"{wp_id}_PD": "PD", f"{wp_id}_BAC": "BAC", f"{wp_id}_pBAC": "pBAC"})
...
def shutdown_callback(data_rows, bagname, csv_path, data, savegoal = True):
    global STOP_THREAD  # Access the global variable
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if recording:
            ...
            for wp_id in data_handler.pastBACs.keys():
                data_row[f'{wp_id}_pBAC'] = data_handler.pastBACs[wp_id]
